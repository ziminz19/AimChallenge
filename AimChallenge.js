...
export class AimChallenge extends Simulation {
  constructor() {
    ...
    this.materials = {
      ...
      ceil: new Material(new Texture_Scroll_X(), {
        ambient: 0.7,
        specularity: 0,
        texture: new Texture('assets/ceil.jpg', 'NEAREST'),
      }),
      ...
      wall: new Material(new Texture_Scroll_X(), {
        ...
        diffusivity: 0.5,
        specularity: 0,
        texture: new Texture('assets/wall.jpg', 'NEAREST'),
      }),
      ...
      rifle: new Material(new Texture_Scroll_X(), {
        ambient: 1,
        texture: new Texture('assets/rifle_texture.png', 'NEAREST'),
      }),
      ...
      collision_box: new Material(new defs.Phong_Shader(), {
  ...
  make_control_panel() {
    ...
    const gravity_controls = this.control_panel.appendChild(
    ...
    this.key_triggered_button(
      '+',
      ['i'],
      () => (this.gravity < 10 ? (this.gravity *= 1.2) : (this.gravity *= 1)),
      undefined,
      undefined,
      undefined,
      gravity_controls
    ...
    this.key_triggered_button(
      '-',
      ['z'],
      () => (this.speed <= 10 ? (this.speed *= 1) : (this.speed -= 10)),
      undefined,
      undefined,
      undefined,
      speed_controls
    ...
    this.live_string((box) => {
      box.textContent = 'Bullet Speed: ' + this.speed;
    }, speed_controls);
    this.key_triggered_button(
      '+',
      ['x'],
      () => (this.speed >= 60 ? (this.speed *= 1) : (this.speed += 10)),
      undefined,
      undefined,
      undefined,
      speed_controls
    ...
    const fire_controls = this.control_panel.appendChild(
      document.createElement('span')
    );
    fire_controls.style.margin = '15px';
    ...
    const reset_controls = this.control_panel.appendChild(
      document.createElement('span')
    );
    reset_controls.style.margin = '15px';
  ...
  update_state(dt) {
    ...
    if (this.fire) {
      this.bullets.push(
        new Body(
          ...
          Mat4.translation(...this.cam.plus(vec3(0, 0, -15))),
          vec3(0, 0, this.speed * -1),
          0
        )
      );
      this.fire = !this.fire;
    ...
    this.bullets = this.bullets.filter(
      (b) => b.center[2] > -200 && b.center[1] > 0 && !b.hit
    );
  ...
  display(context, program_state) {
    ...
    program_state.lights = [
      new Light(light_position1, color(1, 1, 1, 1), 5000),
      new Light(light_position2, color(1, 1, 1, 1), 5000),
    ];
    ...
    this.shapes.crosshair.draw(
      context,
      program_state,
      Mat4.translation(cam_x, cam_y, cam_z - 20),
      this.materials.crosshair
    );

    // Draw Rifle
    let model_transform_rifle = Mat4.translation(
      cam_x + 0.7,
      cam_y - 0.75,
      cam_z - 0.2
    )
      .times(Mat4.rotation((19 * Math.PI) / 18, 0, 1, 0))
      .times(Mat4.rotation(-Math.PI / 23, 1, 0, 0));
    this.shapes.rifle.draw(
      context,
      program_state,
      model_transform_rifle,
      this.materials.rifle
    );

    // Draw walls
    let model_transform_wall_left = Mat4.translation(-51, 0, 0)
      .times(Mat4.scale(0.5, 8, 100))
      .times(Mat4.translation(1, 1, -1));
    this.shapes.side_wall.draw(
      context,
      program_state,
      model_transform_wall_left,
      this.materials.wall
    );
    let model_transform_wall_right = Mat4.translation(50, 0, 0)
      .times(Mat4.scale(0.5, 8, 100))
      .times(Mat4.translation(1, 1, -1));
    this.shapes.side_wall.draw(
      context,
      program_state,
      model_transform_wall_right,
      this.materials.wall
    );
    let model_transform_wall_back = Mat4.translation(-51, 0, -200)
      .times(Mat4.scale(51, 8, 0.5))
      .times(Mat4.translation(1, 1, -1));
    this.shapes.back_wall.draw(
      context,
      program_state,
      model_transform_wall_back,
      this.materials.wall
    );

    // Draw floor and ceiling
    let model_transform_floor = Mat4.translation(-51, -1, 0)
      .times(Mat4.scale(51, 0.5, 100))
      .times(Mat4.translation(1, 1, -1));
    this.shapes.floor.draw(
      context,
      program_state,
      model_transform_floor,
      this.materials.floor
    );
    let model_transform_ceil = Mat4.translation(-51, 16, 0)
      .times(Mat4.scale(51, 0.5, 100))
      .times(Mat4.translation(1, 1, -1));
    this.shapes.ceil.draw(
      context,
      program_state,
      model_transform_ceil,
      this.materials.ceil
    );
